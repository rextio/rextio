...
현재 native 후보는 scalar, `list[...]`, fixed `tuple[...]`, 제한적
`dict[str, int|float]`, `Optional[T]` / `T | None` 타입을 지원합니다. 지원 문법은
...
fixed tuple literal, 제한적 dict read/write, 지원 list item 타입의 `list.append(...)`입니다.
...
dict 지원은 `dict[str, int]`와 `dict[str, float]`로 제한되며, dataclass는 아직 Public 1
native 컴파일 범위 밖입니다.
