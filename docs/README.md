...
- [Unsupported features](unsupported-features.md) — the boundaries of the 0.1.x
  supported subset (published package: **0.1.3** / plugin API 1.3).

## Specs

- [Machine-readable tooling contract](specs/tooling-contract.md) — draft
  (current producer `contract_version` `2.1.0` on core 0.1.3; core 0.1.2
  emitted `2.0.0`; `1.0.0` was PyPI 0.1.1): route taxonomy, `check --json`
...
- [Plugin lowering](specs/plugin-lowering.md) — draft (0.1.1+; plugin API 1.1
  ...
  0.1.3): the claim/lower hook that lets plugins translate covered constructs
  to Rust — plugin annotation vocabulary, expression-level codegen contract,
  boundary ABI, pinned crate injection with consent and report exposure,
  structured `ClaimExpr` / leaves-mode fusion surface, and the plugin
  certification kit.
