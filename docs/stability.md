...
| --- | --- | --- |
| Scalar-helper embedding | Experimental | `--jit` / `[jit] enabled`. Unmarked numeric scalar helpers (single arithmetic return expression) are embedded as internal native functions through the normal checked lowering, so overflow/divide-by-zero raise like any native code. Embedded helpers lower through the checked AOT path; there is no runtime JIT compilation. |
