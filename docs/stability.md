...
| Windows CUDA inventory probe (Train C, unreleased) | Experimental | Bounded validation tool only; it does not create a context, manage memory, launch a kernel, or certify CUDA. Every report has `support_claim: false`. See [Windows CUDA validation](testing/windows-cuda-validation.md). |
