...
| Tooling-contract JSON fields | Experimental | `rextio check --format json` (and `.rextio/reports/check.json`) carries a top-level `contract_version` (`2.2.0` in core 0.1.4) plus per-function route and promotion data, as specified in [the tooling contract](specs/tooling-contract.md). Contract `2.0.0` (core 0.1.2) broke `RXT000` column semantics; `2.1.0` (core 0.1.3) added logger/claim metadata; `2.2.0` adds `marker_kind`, isolated `promotion_assessment`, and reliable function/name ranges while preserving `route`, `native_status`, and `rejection_codes` semantics. Dual-map `2.x` consumers that ignore unknown keys remain compatible; the shape may still be refined until the contract is promoted to stable. |
