...
| --- | --- | --- |
| Scalar-helper embedding | Experimental | `--jit` / `[jit] enabled`. Unmarked numeric scalar helpers (single arithmetic return expression) are embedded as internal native functions through the normal checked lowering, so overflow/divide-by-zero raise like any native code. The former runtime Cranelift hot path was removed (benchmarked strictly slower than the AOT path). |
| Numba external accelerator | Supported (external) | `numba.jit`/`njit`/`vectorize`/`guvectorize` decorators keep a function on the Python fallback cleanly and are labeled in reports. Semantics are Numba's contract (e.g. nopython int overflow wraps), not Rextio's native contract. Not compatible with Nuitka-compiled fallbacks. |
