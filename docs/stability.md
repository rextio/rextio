...
The source tree is the unreleased Rextio **0.1.4 candidate** (plugin API
**1.3**; tooling contract **2.2.0**). The current published package remains
**0.1.3** (PyPI 2026-07-17; tooling contract **2.1.0**), superseding **0.1.2**
(contract **2.0.0**). This page is
...
| Tooling-contract JSON fields | Experimental | `rextio check --format json` (and `.rextio/reports/check.json`) carries a top-level `contract_version` (source candidate `2.2.0`; current PyPI producer `2.1.0`) plus per-function route and promotion data, as specified in [the tooling contract](specs/tooling-contract.md). Contract `2.0.0` (core 0.1.2) broke `RXT000` column semantics; `2.1.0` (core 0.1.3) added logger/claim metadata; source candidate `2.2.0` adds `marker_kind`, isolated `promotion_assessment`, and reliable function/name ranges while preserving `route`, `native_status`, and `rejection_codes` semantics. Dual-map `2.x` consumers that ignore unknown keys remain compatible; the shape may still be refined until the contract is promoted to stable. |
