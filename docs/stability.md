...
contract **2.4.0** (host source/artifact/executable planning from the
intermediate **2.3.0** shape, plus plugin standalone-artifact capability
presence and resolved allow/deny details). Those additions are not yet part of
...
| Tooling-contract JSON fields | Experimental | `rextio check --format json` (and `.rextio/reports/check.json`) carries a top-level `contract_version` (`2.2.0` in published core 0.1.4) plus per-function route and promotion data, as specified in [the tooling contract](specs/tooling-contract.md). Contract `2.0.0` (core 0.1.2) broke `RXT000` column semantics; `2.1.0` (core 0.1.3) added logger/claim metadata; `2.2.0` adds `marker_kind`, isolated `promotion_assessment`, and reliable function/name ranges while preserving `route`, `native_status`, and `rejection_codes` semantics. The unreleased Train C producer (package version still 0.1.4) advances through additive `2.3.0` host source/artifact/executable planning to current producer **`2.4.0`** (plugin standalone-capability presence and generate/build resolved allow/deny details; `lowering_provided` / route / native-status / rejection / promotion-assessment semantics unchanged). Dual-map `2.x` consumers that ignore unknown keys remain compatible; the shape may still be refined until the contract is promoted to stable. |
...
| External pure-Python package source-AOT | Planned | `SourceModule` can describe provenance, but recursive installed-package promotion, license locks, SBOM/provenance gates, and vendoring are not implemented. |
