...
contract **2.5.0** (host source/artifact/executable planning from the
intermediate **2.3.0** shape, plus plugin standalone-artifact capability
presence/resolution in **2.4.0**, plus external-source inventory evidence in
**2.5.0**). Those additions are not yet part of
...
| External pure-Python package source-AOT beyond inventory | Planned | C5.1 can produce preview-only inventory for one exact depth-1 distribution. Project-call linkage, Rust lowering, recursive installed-package promotion, license locks, SBOM/provenance authorization, vendoring, and redistribution are not implemented. |
