"""Bounded C6.2 artifact-evidence models and sidecar helpers.
...
class CargoPackageRef:
    ...
    def __post_init__(self) -> None:
        ...
        if self.package_id:
            object.__setattr__(
                self, "package_id", _bounded_string(self.package_id, "package id")
            )
...
class ArtifactEvidence:
    """Additive ``build.json.artifact_evidence`` record for C6.2 preview."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.status == "unavailable":
            ...
            if self.inputs or self.wheel_entries or self.cargo_packages or self.cargo_dependencies:
        ...
        if self.status == "preview-ready":
            ...
            if (
                ...
                data["cargo_dependencies"] = [
                    item.to_dict() for item in self.cargo_dependencies
                ]
        return data
...
def ensure_sidecar_path_contained(
    path: Path, *, project_root: Path, expected_parent: Path
) -> Path:
...
def _open_pinned_parent_dirfd(parent: Path) -> tuple[int, os.stat_result]:
    ...
    try:
        ...
        if (
            ...
            and (
                link_stat.st_ino != dir_stat.st_ino
                or link_stat.st_dev != dir_stat.st_dev
            )
        ):
            raise ArtifactEvidenceError(
...
def build_cyclonedx_document(
    *,
    ...
    """
    if (
        len(inputs) + len(wheel_entries) + len(cargo_packages) + 1
    ...
    for item in sorted(inputs, key=lambda value: (value.role, value.logical_path, value.sha256)):
        ...
        identity = sha256_hex(
            f"{item.role}|{item.logical_path}|{item.sha256}".encode("utf-8")
        )
    ...
    for wheel_entry in sorted(wheel_entries, key=lambda value: value.name):
        bom_ref = (
            f"urn:rextio:wheel-entry:{wheel_entry.sha256}:"
            f"{sha256_hex(wheel_entry.name.encode('utf-8'))[:16]}"
        )
        components.append(
            {
                "type": "file",
    ...
    for package in sorted(cargo_packages, key=_cargo_package_sort_key):
        ...
        if fingerprint is not None:
            properties.append(
                {"name": "rextio:source_fingerprint", "value": fingerprint}
            )
        if package.features:
            properties.append(
                {"name": "rextio:features", "value": ",".join(package.features)}
            )
    ...
    for component_ref in (subject_ref, *depends_on):
...
def build_intoto_provenance_document(
    *,
    ...
    invocation identity from the wheel hash alone.
    """
