"""Path-free PyO3 build configuration for the frozen Full C6 Alpha.
...
paths implicit inputs.  Full C6 instead supplies one canonical
...
FULL_C6_PYO3_CONFIG_DOMAIN = "rextio.full-c6-pyo3-config.v1"
...
class FullC6Pyo3ConfigIdentity:
    ...
    def __post_init__(self) -> None:
        if self.domain != FULL_C6_PYO3_CONFIG_DOMAIN:
            raise ValueError("Full C6 PyO3 config domain is invalid")
        if self.scope != FULL_C6_PYO3_CONFIG_SCOPE:
            raise ValueError("Full C6 PyO3 config scope is invalid")
        if self.target_triple not in _SUPPORTED_TARGETS:
            raise ValueError("Full C6 PyO3 config target is unsupported")
        if type(self.content) is not bytes or self.content != _canonical_content():
            raise ValueError("Full C6 PyO3 config bytes are not canonical")
        if type(self.size) is not int or self.size != len(self.content):
            raise ValueError("Full C6 PyO3 config size is invalid")
        if (
            _SHA256_RE.fullmatch(self.sha256) is None
            or not hmac.compare_digest(self.sha256, hashlib.sha256(self.content).hexdigest())
        ):
            raise ValueError("Full C6 PyO3 config SHA-256 is invalid")
...
def capture_full_c6_pyo3_config(target_triple: str) -> FullC6Pyo3ConfigIdentity:
    ...
    if struct.calcsize("P") * 8 != 64:
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config requires a 64-bit interpreter")
    if getattr(sys, "abiflags", "") not in {"", None} or hasattr(sys, "gettotalrefcount"):
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config rejects a non-release CPython ABI")
...
def materialize_full_c6_pyo3_config(
    ...
    if type(identity) is not FullC6Pyo3ConfigIdentity:
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config identity has an invalid type")
    root = Path(directory)
    try:
        observed_root = os.lstat(root)
    except OSError as exc:
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config root is unavailable") from exc
    if stat.S_ISLNK(observed_root.st_mode) or not stat.S_ISDIR(observed_root.st_mode):
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config root is unsafe")
    ...
    try:
        ...
        try:
            ...
            while view:
                ...
                if written <= 0:
                    raise FullC6Pyo3ConfigError("Full C6 PyO3 config write failed")
                view = view[written:]
            os.fsync(descriptor)
            opened = os.fstat(descriptor)
            if not stat.S_ISREG(opened.st_mode) or opened.st_nlink != 1:
                raise FullC6Pyo3ConfigError("Full C6 PyO3 config output is unsafe")
        finally:
            os.close(descriptor)
        ...
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config could not be materialized") from exc
    return path
    ...
    try:
        ...
        try:
            ...
            if (
                ...
                raise FullC6Pyo3ConfigError("Full C6 PyO3 config file is unsafe")
        ...
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config could not be verified") from exc
    if (
        ...
        raise FullC6Pyo3ConfigError("Full C6 PyO3 config changed after capture")
