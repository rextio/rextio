...
Contract 2.x (position semantics from 2.0.0; current producer 2.5.0) standardizes
...
def test_tooling_contract_version_is_major_2() -> None:
    ...
    # 2.5.0 adds preview-only external-source evidence on the same major.
    assert TOOLING_CONTRACT_VERSION == "2.5.0"
...
def test_rxt000_column_in_check_json_report(tmp_path: Path) -> None:
    ...
    assert report["contract_version"] == "2.5.0"
