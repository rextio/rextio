...
Contract 3.0.0 retains the position semantics introduced in 2.0.0: every
diagnostic column (including RXT000) is a 0-based UTF-8 byte offset.
...
def test_tooling_contract_version_is_major_3() -> None:
    # Major 3 changes artifact terminology without reverting the major-2
    # diagnostic-position semantics.
    assert TOOLING_CONTRACT_VERSION == "3.0.0"
...
def test_rxt000_column_in_check_json_report(tmp_path: Path) -> None:
    ...
    assert report["contract_version"] == "3.0.0"
