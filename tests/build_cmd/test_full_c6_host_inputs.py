...
def test_record_inventory_accepts_only_source_bound_pip_bytecode_rows(
    tmp_path: Path,
) -> None:
    root, record, module = _write_record_install(tmp_path)
    nested_source = root / "rextio" / "nested" / "worker.py"
    nested_source.parent.mkdir()
    nested_source.write_bytes(b"VALUE = 2\n")
    bytecode_members = (
        "rextio/__pycache__/__init__.cpython-311.pyc",
        "rextio/__pycache__/__init__.cpython-311.opt-1.pyc",
        "rextio/nested/__pycache__/worker.cpython-311.opt-2.pyc",
    )
    for logical in bytecode_members:
        path = root.joinpath(*logical.split("/"))
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(b"pip generated bytecode")
    _extend_record(
        record,
        [
            [
                "rextio/nested/worker.py",
                _record_hash(nested_source.read_bytes()),
                str(nested_source.stat().st_size),
            ],
            *[[logical, "", ""] for logical in bytecode_members],
        ],
    )

    identity = host_inputs._capture_record_backed_rextio_identity(
        distribution_root=root,
        record_path=record,
        module_file=module,
        version=__version__,
